...
const EmailInput = () => {
    ...
    const handleInput = async (e) => {
        e.preventDefault();
        const res = await apiRequest("/api/register", "POST", { name, email }).then(r => r.text());
        console.log("Received this: " + res);
    }

    return (
        <div className="flex flex-col items-center justify-center mx-4 mb-4">
            <input id="name-input" type="text" className="text-xl w-md my-2 py-4 px-6 rounded-2xl ring-1 ring-emerald-950 border border-emerald-100
                    dark:bg-slate-800 dark:text-white dark:border-emerald-700
                    placeholder:text-xl" placeholder="Your Name" 
